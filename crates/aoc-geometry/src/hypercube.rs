...
use crate::point_coordinate::{PointCoordinate, minimum_point_coordinate_value};
...
impl<T: PointCoordinate, const N: usize> HyperCube<T, N> {
        ...
        U: VectorCoordinate,
    {
        ...
        let most_negative_point = Point::new([minimum_point_coordinate_value::<T>(); N]);
        if Vector::<i64, N>::from_points(&most_negative_point, vertex).manhattan_distance()
            < Vector::<i64, N>::from_points(&most_negative_point, &vertex2).manhattan_distance()
        {
            Self {
                min_vertex: *vertex,
                max_vertex: vertex2,
    ...
    pub fn from_opposite_vertices(vertex1: &Point<T, N>, vertex2: &Point<T, N>) -> Self {
        HyperCube::from_vertex_and_diagonal(
            vertex1,
            &Vector::<i64, N>::from_points(&vertex1, &vertex2),
        )
    }
